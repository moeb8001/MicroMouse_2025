...
CONFIG: Dict[str, Any] = {
    ...
    "min_wafer_coverage": 0.30,        # skip cells mostly outside the wafer
    ...
    # ── Particle detection ───────────────────────────────────────────────────
    "tophat_kernel_px":    21,     # structuring-element diameter (odd)
    "particle_thresh":     18,     # intensity after top-hat (0–255)
    "min_particle_area_px": 4,
    "max_particle_area_px": 5000,
    "min_circularity":     0.30,   # 0 disables; ~0.5 rejects scratches
    "detect_bright":       True,
    "detect_dark":         True,

    # ── Matching before/after particles ──────────────────────────────────────
    "match_radius_um":     6.0,    # max distance to pair a before↔after particle

    # ── Global deduplication across tile borders ─────────────────────────────
    "dedup_radius_um":     3.0,

    # ── Confidence cross-check ───────────────────────────────────────────────
    "confidence_crop_px":   31,
    "confidence_low_thresh": 0.60,

    # ── Edge exclusion (ignore the noisy wafer rim) ──────────────────────────
    "edge_exclusion_mm":   1.5,
...
#  PARTICLE DETECTION
# ══════════════════════════════════════════════════════════════════════════════
def detect_in_section(gray: np.ndarray, mask: np.ndarray,
                      cfg: Dict[str, Any]) -> List[Particle]:
    """Detect particles in one section image using top-hat + threshold."""
    k = cfg["tophat_kernel_px"] | 1
    ...
    for enabled, op, kind in [
        ...
        if not enabled:
            continue
        hat = cv2.morphologyEx(gray, op, se)
        hat[mask == 0] = 0

        _, bw = cv2.threshold(hat, cfg["particle_thresh"], 255,
                              cv2.THRESH_BINARY)
        # morphology cleanup
        ...
        for cnt in contours:
            ...
            cx = M["m10"] / M["m00"]
            cy = M["m01"] / M["m00"]
            peak = float(hat[max(0, int(cy) - 1):int(cy) + 2,
                             max(0, int(cx) - 1):int(cx) + 2].max())
            particles.append(Particle(
                id=pid, x_px=cx, y_px=cy, area_px=area, circularity=circ,
                peak_intensity=peak, kind=kind))
            pid += 1
    return particles


# ══════════════════════════════════════════════════════════════════════════════
#  SECTION-LEVEL MATCHING (new / existing / removed)
# ══════════════════════════════════════════════════════════════════════════════
def match_section_particles(before: List[Particle],
                            after: List[Particle],
                            match_radius_px: float
                            ) -> Tuple[List[Particle], List[Particle],
                                       List[Particle]]:
    """Returns (existing_in_after, new, removed)."""
    ...
    coords_b = np.array([(p.x_px, p.y_px) for p in before])
    tree_b = cKDTree(coords_b)
    matched_b = set()

    for p in after:
        d, idx = tree_b.query([p.x_px, p.y_px])
        if d <= match_radius_px:
            p.status = "existing"
            existing.append(p)
            matched_b.add(int(idx))
        else:
            p.status = "new"
            new.append(p)
...
def process_section(sec: GridSection,
    ...
    try:
        ...
        # --- build section-local wafer mask ---
        H, W = before_pp.shape
        yy, xx = np.ogrid[:H, :W]
        ...
        # --- detect in both ---
        before_parts = detect_in_section(before_pp, section_mask, cfg)
        after_parts  = detect_in_section(after_pp,  section_mask, cfg)

        # --- classify ---
        match_radius_px = cfg["match_radius_um"] / geom.um_per_px
        existing, new, removed = match_section_particles(
            before_parts, after_parts, match_radius_px)
...
#  CONFIDENCE CROSS-CHECK
# ══════════════════════════════════════════════════════════════════════════════
def cross_check_confidence(new_parts: List[Particle],
                           lazy_before: LazyImage,
                           cfg: Dict[str, Any]) -> None:
    if not new_parts:
        return
    half = cfg["confidence_crop_px"] // 2
    H, W = lazy_before.shape
    for p in new_parts:
        ...
        if x1 - x0 < 5 or y1 - y0 < 5:
            p.confidence = 0.5
            continue
        crop = lazy_before.crop(x0, y0, x1, y1).astype(np.float32)
        k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7))
        if p.kind == "bright":
            hat = cv2.morphologyEx(crop, cv2.MORPH_TOPHAT, k)
        else:
            hat = cv2.morphologyEx(crop, cv2.MORPH_BLACKHAT, k)
        peak_b = float(hat.max())
        peak_a = max(1.0, p.peak_intensity)
        ratio = peak_b / peak_a
        p.confidence = float(np.clip(1.0 - ratio, 0.0, 1.0))
...
def process_wafer(before_path: Path, after_path: Path,
    ...
    # --- confidence cross-check on NEW particles ---
    cross_check_confidence(all_new, lazy_b, cfg)
