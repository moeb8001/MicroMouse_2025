...
CONFIG: Dict[str, Any] = {
    ...
    # CLAHE is GATED by local tile variance: it is only applied on tiles
    # whose wafer-masked standard deviation is above `clahe_min_std`.
    # CLAHE on a low-variance tile (wipe, blank silicon) manufactures
    # contrast from sensor noise, which the downstream top-hat then
    # interprets as thousands of "particles" — the main root cause of
    # over-detection in the previous run.  By vetoing CLAHE on flat tiles
    # we keep contrast-boosting exactly where it actually helps.
    "use_clahe":              True,
    "clahe_min_std":          6.0,    # skip CLAHE if tile std < this
    "clahe_clip_limit":       2.0,
    "clahe_tile_grid":        8,
    # Top-hat kernel: rule of thumb is diameter >= 8–10 × expected particle
    # diameter.  A typical particle on these wafers is ~5–15 px wide, so the
    # kernel should be at least ~120 px.  The previous 35 px default
    # matched the silicon texture scale, so the background was itself being
    # detected.  151 px is safely above both the particle size AND the
    # stitched-tile texture wavelength.
    "bg_tophat_kernel_px":    151,
    ...
    # ── Isolation / local-peak check  (background blob rejection) ────────────
    # Real particles are SHARP local extrema: the residual at the centroid
    # should clearly dominate a ring around the particle.  Background
    # texture blobs have no such isolation — the surrounding ring is just
    # as bright.  We require:
    #     residual_at_center  >=  isolation_ratio * mean(ring)  + isolation_abs
    # Both tests must pass.
    "isolation_enable":       True,
    "isolation_ring_inner_px": 4,
    "isolation_ring_outer_px": 12,
    "isolation_ratio":        1.6,
    "isolation_abs":          3.0,

    # ── Tile-level density veto (hard safety net) ────────────────────────────
    # Real wafer particle densities are extremely low — tens, not thousands,
    # per tile.  If a tile reports more than `max_detections_per_tile`
    # detections after all filters, the ENTIRE tile is discarded as
    # background / artefact.  This guarantees that "yellow-carpet" failures
    # cannot propagate into the particle map even if every other safeguard
    # is bypassed.
    "max_detections_per_tile": 200,
...
# ══════════════════════════════════════════════════════════════════════════════
#  IMAGE DECODING  —  OpenCV-first, Pillow-fallback
# ══════════════════════════════════════════════════════════════════════════════
def _decode_gray_image(path: Path) -> np.ndarray:
    """Decode a large image as single-channel uint8.

    Strategy: try OpenCV first, fall back to Pillow only if OpenCV can't
    handle the file.  OpenCV's JPEG decoder is noticeably SMOOTHER than
    Pillow's — it preserves less of the wafer's micro-texture, which
    dramatically reduces the top-hat residual on background silicon and
    therefore cuts false positives before detection even starts.
    Pillow preserves more high-frequency texture, and when that texture is
    fed into a top-hat with a small kernel the detector decides that
    EVERYTHING is a particle.
    """
    # 1. OpenCV (preferred — smoother decode)
    try:
        arr = cv2.imread(str(path), cv2.IMREAD_GRAYSCALE)
        if arr is not None and arr.size > 0:
            return arr
    except Exception as e:
        logger.warning("OpenCV failed to decode %s (%s) — falling back to Pillow",
                       path.name, e)
    # 2. Pillow fallback
    try:
        ...
        raise FileNotFoundError(
            f"Could not decode {path} with OpenCV or Pillow: {e}")
...
class LazyImage:
    ...
    def open(self) -> None:
        if self.use_mm:
            ...
            self._arr = _decode_gray_image(self.path)
    ...
    def thumbnail(self, max_side: int = 2000) -> np.ndarray:
        ...
        # Strided read from the already-decoded backend is the smoothest path
        # (identical decoder characteristics to the full image).
        try:
            step = max(1, int(ds))
            rough = np.asarray(self._arr[::step, ::step])
            return cv2.resize(rough, (sw, sh), interpolation=cv2.INTER_AREA)
        except Exception:
            pass
        # Fallback: Pillow draft decode
        try:
    ...
    def _get_or_make_memmap(self) -> np.memmap:
        ...
        t0 = time.time()
        arr = _decode_gray_image(self.path)
        sh = arr.shape
        mm = np.memmap(dat, dtype=np.uint8, mode="w+", shape=sh)
        # Stripe-wise copy: writing a huge ndarray into a memmap with a single
        # slab assignment can trigger pathological cache / page-fault behaviour
        # on some Windows setups, producing slightly different pixel values
        # from the "direct" loader and in turn changing top-hat responses.
        # Horizontal stripes match the behaviour of the older, trusted loader.
        stripe_rows = 1024
        H = sh[0]
...
def normalise_tile(gray: np.ndarray, mask: np.ndarray,
                   cfg: Dict[str, Any]) -> Tuple[np.ndarray, bool]:
    """Per-tile illumination normalisation.

    1. CLAHE — ONLY if the tile has enough real variation.  On a flat
       low-contrast tile (wipe, empty silicon), CLAHE manufactures contrast
       out of sensor noise which the downstream top-hat then detects as a
       carpet of fake particles.  Gating by masked std ensures CLAHE runs
       only where there is actual structure worth equalising.
    2. Gentle Gaussian smoothing.
    3. Mask-out non-wafer pixels.

    Returns (normalised_image, clahe_applied_flag).
    """
    out = gray.copy()
    clahe_applied = False
    if cfg.get("use_clahe", True):
        valid = mask > 0
        if valid.sum() >= 100:
            local_std = float(gray[valid].std())
            if local_std >= float(cfg.get("clahe_min_std", 6.0)):
    ...
    out[mask == 0] = 0
    return out, clahe_applied


def _is_isolated_peak(residual: np.ndarray, ix: int, iy: int,
                      cfg: Dict[str, Any]) -> Tuple[bool, float, float]:
    """Local-peak / isolation test.

    A true particle is a sharp local extremum: its centre residual should
    clearly dominate a ring of pixels surrounding it.  Background texture
    blobs fail this because the neighbourhood is just as bright.
    Returns (is_isolated, centre_value, ring_mean_value).
    """
    h, w = residual.shape
    r_in  = int(cfg.get("isolation_ring_inner_px", 4))
    r_out = int(cfg.get("isolation_ring_outer_px", 12))
    y0 = max(0, iy - r_out); y1 = min(h, iy + r_out + 1)
    x0 = max(0, ix - r_out); x1 = min(w, ix + r_out + 1)
    if y1 - y0 < 3 or x1 - x0 < 3:
        return False, 0.0, 0.0
    patch = residual[y0:y1, x0:x1]
    yy, xx = np.ogrid[y0:y1, x0:x1]
    d2 = (yy - iy) ** 2 + (xx - ix) ** 2
    ring_mask = (d2 >= r_in * r_in) & (d2 <= r_out * r_out)
    if ring_mask.sum() < 6:
        return False, 0.0, 0.0
    ring_vals = patch[ring_mask]
    ring_mean = float(ring_vals.mean())
    # centre = small neighbourhood peak (robust to sub-pixel centroid)
    cy0 = max(0, iy - 1); cy1 = min(h, iy + 2)
    cx0 = max(0, ix - 1); cx1 = min(w, ix + 2)
    centre_val = float(residual[cy0:cy1, cx0:cx1].max())
    ratio_ok = centre_val >= float(cfg.get("isolation_ratio", 1.6)) * max(1.0, ring_mean)
    abs_ok   = (centre_val - ring_mean) >= float(cfg.get("isolation_abs", 3.0))
    return (ratio_ok and abs_ok), centre_val, ring_mean
...
def detect_particles_in_tile(gray: np.ndarray, mask: np.ndarray,
    ...
    Pipeline per tile:
        1. White top-hat with a kernel much larger than any real particle
           (so the background texture is mathematically suppressed).
        2. Robust k·MAD threshold on the residual.
        3. Area + circularity filters on connected components.
        4. Isolation / local-peak test (background blob rejection).
        5. Tile-level density veto: if the surviving count is absurdly high
           for one tile, discard ALL of them — it's a background artefact.
    """
    valid = mask > 0
    if valid.sum() < 200:
        return []
    ...
    for enabled, op in [
        ...
        for cnt in contours:
            ...
            if not (0 <= iy < h and 0 <= ix < w):
                continue
            # ---- isolation / local-peak test ----
            if cfg.get("isolation_enable", True):
                isolated, centre_val, ring_mean = _is_isolated_peak(
                    residual, ix, iy, cfg)
                if not isolated:
                    continue
                peak = centre_val
    ...
    # ---- tile-level density veto (hard safety net) -------------------------
    max_det = int(cfg.get("max_detections_per_tile", 200))
    if max_det > 0 and len(detections) > max_det:
        # Everything in this tile is suspicious — it's background, not particles.
        return []
    return detections
...
def detect_all_particles(lazy: LazyImage, geom: WaferGeometry,
                         ...
                         label: str,
                         vetoed_tiles: Optional[List[str]] = None,
                         clahe_skipped_tiles: Optional[List[str]] = None
                         ) -> List[Particle]:
    ...
    for k, tile in enumerate(tiles):
        ...
        norm, clahe_applied = normalise_tile(gray, mask, cfg)
        if not clahe_applied and clahe_skipped_tiles is not None:
            clahe_skipped_tiles.append(tile.label)
        dets = detect_particles_in_tile(norm, mask, cfg)
        # If density veto triggered, detect_particles_in_tile already returned [].
        # Record the event so it shows up in the review summary.
        if max_det > 0 and len(dets) == 0 and vetoed_tiles is not None:
            # We don't know WHY dets is empty — could be "really nothing there".
            # Re-run detection temporarily with the veto disabled to tell.
            cfg_no_veto = dict(cfg)
            cfg_no_veto["max_detections_per_tile"] = 0
            raw = detect_particles_in_tile(norm, mask, cfg_no_veto)
            if len(raw) > max_det:
                vetoed_tiles.append(tile.label)
...
def _downsample_for_panel(lazy: LazyImage, max_side: int) -> Tuple[np.ndarray, float]:
    ...
    ds = max(1.0, max(H, W) / max_side)
    sw, sh = int(W / ds), int(H / ds)
    # Downsample the already-decoded backend — same decoder characteristics
    # as the rest of the pipeline (OpenCV-first if memmapped from cv2).
    step = max(1, int(ds))
    rough = np.asarray(lazy._arr[::step, ::step])
    thumb = cv2.resize(rough, (sw, sh), interpolation=cv2.INTER_AREA)
    return thumb, ds
...
def save_review_summary(path: Path, wafer_name: str,
    ...
    lines.append("")
    vb = info.get("vetoed_tiles_before", [])
    va = info.get("vetoed_tiles_after",  [])
    cb = info.get("clahe_skipped_before", [])
    ca = info.get("clahe_skipped_after",  [])
    lines.append("Safety nets")
    lines.append("-----------")
    lines.append(f"  Density-vetoed tiles  : before={len(vb)}  after={len(va)}")
    lines.append(f"  CLAHE skipped (flat)  : before={len(cb)}  after={len(ca)}")
    if vb or va:
        shown = sorted(set(vb) | set(va))[:20]
        lines.append(f"  Examples: {', '.join(shown)}"
                     + (" ..." if (len(vb) + len(va)) > 20 else ""))
    lines.append("")
...
def process_wafer_pair(pre_path: Path, post_path: Path, cfg: Dict[str, Any]):
    ...
    vetoed_b: List[str] = []; clahe_skipped_b: List[str] = []
    vetoed_a: List[str] = []; clahe_skipped_a: List[str] = []
    before = detect_all_particles(lazy_b, geom, tiles, cfg, "BEFORE",
                                  vetoed_tiles=vetoed_b,
                                  clahe_skipped_tiles=clahe_skipped_b)
    after  = detect_all_particles(lazy_a, geom, tiles, cfg, "AFTER",
                                  vetoed_tiles=vetoed_a,
                                  clahe_skipped_tiles=clahe_skipped_a)
    logger.info("  BEFORE: %d tile(s) density-vetoed, %d CLAHE-skipped",
                len(vetoed_b), len(clahe_skipped_b))
    logger.info("  AFTER : %d tile(s) density-vetoed, %d CLAHE-skipped",
                len(vetoed_a), len(clahe_skipped_a))

    added, removed, unchanged, info = compare_particle_maps(before, after, cfg)
    info["vetoed_tiles_before"]       = vetoed_b
    info["vetoed_tiles_after"]        = vetoed_a
    info["clahe_skipped_before"]      = clahe_skipped_b
    info["clahe_skipped_after"]       = clahe_skipped_a
